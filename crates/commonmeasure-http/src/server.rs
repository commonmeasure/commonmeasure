...
//! connection.
//!
//! Its job is to be a real upstream on loopback. Recorded external bytes may
//! stand in for an uncontrolled network, but a test-only implementation may
//! not stand in for our own code, so a test that wants a
