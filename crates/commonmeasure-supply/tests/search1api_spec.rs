...
//!
//! No recon capture of Search1API is committed, so these tests need no
//! credential. Each test starts a loopback origin serving a documented-shape
//! body, points the adapter's base URL at it, and asserts on both directions —
//! the request the adapter put on the wire, and the envelopes and charge its
//! parser derived from the reply. Nothing in the path is a stand-in but the
//! origin, which stands in for the network, never for the code under test.
