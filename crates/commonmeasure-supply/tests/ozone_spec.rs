...
//! response shapes.
//!
//! Ozone's verification state and the artefacts behind it are recorded in
//! `docs/knowledge-base/provider-verification.md`; no recon capture is
//! committed, so there is nothing to replay and these tests need no
//! credential. Each test starts a loopback origin serving a documented-shape
//! response (`https://api.ozone.live/openapi.yaml`), points the adapter's base
//! URL at it, and asserts on both directions — the request the adapter put on
//! the wire and the envelopes and charge its parser derived from the reply.
//! Nothing in the path is a stand-in but the origin, which is the
//! substitution `AGENTS.md` permits.
//!
