...
//!
//! You.com's verification state and the artefacts behind it are recorded in
//! `docs/knowledge-base/provider-verification.md`; no recon capture is
//! committed, so these tests exercise the parser over the documented shapes
//! rather than recorded bytes, and need no credential. Nothing in the path is a stand-in — the
