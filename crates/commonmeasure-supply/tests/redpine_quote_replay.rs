...
//! provider actually returned on 4 August 2026 (`demo/recon/redpine/`).
//!
...
//! parsers are the ones a live call uses. Only the origin is local, which is
...
fn quote_reads_the_recorded_price_billing_note_and_balance_without_buying() {
    ...
    // provider's own unit, marked quoted, with the R1 disagreement named.
    ...
    assert!(
        native
            ...
            .unwrap_or_default()
            .contains("cost_usd"),
        "the note names the twenty-fold disagreement (register R1)"
    );
