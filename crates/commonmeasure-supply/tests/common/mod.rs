...
//! `recon/`. Nothing else in the path is a stand-in: the client, the
//! framing and the parser under test are the ones a live call uses. The origin
...
impl Origin {
    /// Serve one recorded capture from `recon/`: the recorded status, and
...
fn recon() -> std::path::PathBuf {
    std::path::Path::new(env!("COMMONMEASURE_EVIDENCE_DIR")).join("recon")
}
...
/// declares for it; `relative` is the path beneath `recon/`.
///
...
pub fn verify_capture(relative: &str, capture: &Value) -> Result<(), String> {
    let Some(expected) = declared_hashes().get(relative).cloned() else {
        return Err(format!(
            "{relative} is in no manifest; add it to recon/replay-manifest.json if a run serves it, or recon/fixture-manifest.json if only a test does"
        ));
    };
