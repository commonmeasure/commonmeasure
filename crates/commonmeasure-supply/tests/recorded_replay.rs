...
//! `recon/`, points the adapter's base URL at it, and asserts on both
...
//! live call uses. Only the origin is local: it stands in for the network,
//! never for the code under test, and replay makes the same substitution.
//!
...
/// spec-verified only and this test exercises it over the documented shape
/// rather than recorded bytes. A result without the field must stay undated.
#[test]
...
/// `exa/exa-contents.json` is not in `recon/replay-manifest.json` and no
...
/// `firecrawl/firecrawl-scrape.json` is not in `recon/replay-manifest.json`
