//! No recon capture of Linkup is committed, so these tests exercise the parser
//! over Linkup's documented `searchResults` response shape through the real
//! transport and a loopback origin, and need no credential. Nothing in the
//! path is a stand-in — the client, the framing, the parser and the charge
//! rules are the ones a live call would use. Only the origin is local: it
