...
//! machine-readable anywhere in the responses (register R4) and declaring the
...
impl RedpineAdapter {
    ...
    /// at a loopback origin serving `demo/recon/redpine/`. The credential is
    ...
    pub fn new(base_url: &str, api_key: &str) -> Self {
        Self {
            ...
            // under the operator's own credential (`DECISIONS.md` §Product).
...
/// (`demo/recon/redpine/redpine-inspect-sample-2.json`) — prose framing, so
...
/// register R1: nothing in the responses says which figure a funded account
/// is charged.
        ...
        native: Some(NativeCharge {
            ...
            note: Some(
                "inspect-tool's pricing annotation for media--sample. The confirmed payload's \
                 ...
                 (docs/knowledge-base/unverified-assumptions.md R1)."
                    .to_owned(),
            ),
        }),
    }
...
/// (`demo/recon/redpine/redpine-balance-*.json`), so the honest unit is the
...
/// note saying the receipt itself does not name one (register R1; no
        ...
        native: amount.map(|amount| NativeCharge {
            ...
            note: Some(format!(
                "The receipt does not name its unit; credits is the unit Redpine's balance and \
                 pricing annotations use (docs/knowledge-base/unverified-assumptions.md R1). \
                 {receipt}."
            )),
        }),
    }
    ...
    let envelopes = results_array(payload, "/mentions", endpoint)?
        ...
        .filter_map(|(index, mention)| {
            ...
            Some(ContextEnvelope {
                ...
                // responses (register R4). `_meta.data_source` names a data
                // partnership, which is provenance, not a licence; it stays
                // in the payload and is never promoted into a rights claim.
