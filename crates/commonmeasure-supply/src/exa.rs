...
impl ExaAdapter {
    ...
    /// server replaying `demo/recon/exa/`. The credential is held here and
...
impl SupplyAdapter for ExaAdapter {
    ...
    /// them exactly (`docs/knowledge-base/provider-verification.md` §Exa:
    /// `costDollars.total` 0.007 and 0.001). If Exa's pricing page moves,
    /// this moves with it and the verification record says when it was last
    /// checked; the allowance ledger reconciles the reservation against the
    /// observed charge either way, so a drifted figure is visible on every
    /// receipt.
...
fn envelopes_from(body: &Value, endpoint: &str) -> Result<Vec<ContextEnvelope>, SupplyError> {
    ...
    Ok(envelopes_from_results(results, |item| {
        Some(ResultFields {
            ...
            // (`demo/recon/exa/`), so on those bytes this maps nothing — the
            // mapping itself is spec-verified only, and the date, where one
            // arrives, is Exa's claim
            // (`docs/knowledge-base/unverified-assumptions.md` E3).
            declared_date: item
                .get("publishedDate")
                .and_then(Value::as_str)
...
/// breakdown.
///
/// Observed 1 August 2026 (`demo/recon/exa/`): a combined search-and-contents
