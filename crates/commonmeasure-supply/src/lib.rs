...
//! verification record (`docs/knowledge-base/provider-verification.md`),
...
//! under test are then the ones a live run uses. That is the boundary
//! `ROADMAP.md` requires recorded replay to enter through. A skill has no
//! recorded form at all: a produced result is not a response anybody
//! published, so the skill tests execute real code under the real limits and
...
/// capabilities, which is what keeps `AGENTS.md`'s rule enforceable in code:
/// a skill adapter never implements `search`, `query` or `quote`, so nothing
/// can ask a local execution for search semantics, a supplier price or a
/// licence it has no way to state.
pub trait SupplyAdapter {
    ...
    /// observed receipt (`DECISIONS.md` §Delegated authority and fleet management). Implement it only where the
    /// published price has been verified against a dated observed charge —
    /// a stale figure here would reserve the wrong amount silently, so each
    /// implementation names its evidence.
    fn published_price(
        &self,
