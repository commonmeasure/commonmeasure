...
fn envelopes_from(body: &Value, endpoint: &str) -> Result<Vec<ContextEnvelope>, SupplyError> {
    ...
    Ok(envelopes_from_results(results, |item| {
        Some(ResultFields {
            ...
            // the recorded general search carries none.
...
/// unknown charge, which the ledger treats as unpriced, never as free. A call
/// whose only URL failed is the documented zero and is quoted as such.
