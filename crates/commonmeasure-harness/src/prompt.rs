...
//! system (`DECISIONS.md` §Source declarations). The hook therefore records
//! two different things. For each URL in the prompt it records a hash, so a
//! later mediated crossing can say whether the user or the agent named the
//! source without any prompt text entering the record. For pasted material
//! it reads the statements the paste itself carries: an RSL licence in
//! HTML, a `Content-Usage` line in a pasted HTTP response, and a C2PA
//! manifest embedded in text under Annex A.8 or in HTML under A.7, read
//! with the C2PA SDK. Plain text with nothing embedded is recorded as
//! carrying no embedded statement, which is unknown, never disallow.
//!
