...
//!
//! Four mechanisms are read (`docs/knowledge-base/source-declarations.md`):
//! the `Content-Usage` response header and `robots.txt` rule of the IETF
//! AI-preferences attachment draft, Cloudflare's `Content-Signal` line, and an
//! RSL licence discovered from a `robots.txt` `License:` line or a
//! `Link: rel="license"` header. Each produces statements; a statement is a
//! preference, not a licence and not enforcement (`DECISIONS.md` §Source
//! declarations). Statements are recorded with their source and combined
...
/// The product token publishers address in `robots.txt`
/// (`PRODUCT.md` §Naming). Chosen once and never renamed.
pub const PRODUCT_TOKEN: &str = "CommonMeasureBot";

/// The profile URI of the Content Telemetry reporting binding an RSL
/// `<reporting>` element names (`docs/knowledge-base/source-declarations.md`
...
/// knows: `Content-Usage` rules, `Content-Signal` lines and `License:`
/// directives. Unknown lines are ignored, as the protocol requires.
///
...
impl RobotsFile {
    /// Select the group for `token` (the `*` group as fallback) and read what
    /// it says about `path`. `path` is the request target: path and query.
    pub fn read(&self, token: &str, path: &str) -> RobotsReading {
        let wanted = token.to_ascii_lowercase();
        let named = self
            .groups
            .iter()
            .find(|group| group.agents.contains(&wanted));
        let fallback = self
            .groups
            .iter()
            .find(|group| group.agents.iter().any(|a| a == "*"));
        let (group, name) = match (named, fallback) {
            (Some(group), _) => (group, token.to_owned()),
        ...
        let decided = longest_match(
            group.rules.iter().map(|rule| (rule.pattern.as_str(), rule)),
            path,
        )
        ...
        if crawlable {
            ...
            // section 3.1).
            let candidates: Vec<(usize, &(Option<String>, String))> = group
                .content_usage
                .iter()
                .filter_map(|rule| {
            ...
            let (signals, scope) = if group.content_signal.is_empty() {
                (&self.global_content_signal, None)
            } else {
                (&group.content_signal, Some(name.as_str()))
            };
            statements.extend(signal_statements(signals, scope));
        }
        let licences = if group.licences.is_empty() {
            self.global_licences.clone()
        } else {
            group.licences.clone()
        };
        RobotsReading {
            group: Some(name),
...
pub struct LicenceTerms {
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub payment: Option<RslPayment>,
    /// Every reporting requirement of the licences that permit AI input.
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub reporting: Vec<RslReporting>,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub server: Option<String>,
    ...
    for category in Category::ALL {
        ...
        if let Some(licence) = allowed_by {
            ...
            if category == Category::AiInput {
                ...
                terms.reporting = licence.reporting.clone();
            }
