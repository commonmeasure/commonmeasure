...
pub struct PolicyFile {
    ...
    /// Delegated authority keyed by an authenticated identity: an
    /// operating-system user, the subject of a verified access token or an
    /// edge-issued token ([`BindingKey`]). An empty list leaves the policy
    /// scoped by directory alone, resolving exactly as a policy with no
    /// principals.
...
pub struct TermsDeclaration {
    /// The host this reference concerns, exactly; a subdomain needs its own
    /// entry.
    pub host: String,
    /// The operator's reference for the basis. Recorded as the licence
    /// reference on every crossing the terms govern and projected as
    /// `license_ref`.
    pub reference: String,
    /// Whether the assessed basis requires usage reporting to the source.
...
/// `os_user`, `subject` and `edge_token`; the loader refuses one naming more
/// or none, so a document can never leave the key a binding selects on to
/// interpretation ([`PrincipalBinding::key`]).
#[derive(Debug, Clone, Deserialize, Serialize, JsonSchema)]
#[serde(deny_unknown_fields)]
pub struct PrincipalBinding {
    pub principal: String,
    /// The effective numeric operating-system user id, read from the process
    /// rather than an environment variable.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub os_user: Option<u32>,
    /// The `sub` claim of an access token the hosted edge verified: the hub's
    /// stable, opaque user id, never an email address. The organisation's
    /// member list maps it to a person.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub subject: Option<String>,
    /// The label of a bearer token the edge itself issued for a host with no
    /// OAuth. The edge stores the token as a hash; the label is what a
    /// binding and the record name.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    /// reconciles at the purchase. An empty list declares no cumulative
    /// limit, exactly as before allowances existed.
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
...
pub enum AuthenticationBasis {
    ...
    OsUser,
    /// The subject of an access token the hosted edge verified against the
    /// issuer pinned at enrolment. The person signed in at the hub.
    OauthSubject,
    /// A bearer token the edge itself issued for one named principal, for a
    /// host with no OAuth. Weaker than the other two: the secret sits in the
    /// host's own settings, readable by that host's administrators.
    EdgeToken,
...
impl AuthenticationBasis {
    ...
    pub fn as_str(self) -> &'static str {
        match self {
            Self::OsUser => "os_user",
            Self::OauthSubject => "oauth_subject",
            Self::EdgeToken => "edge_token",
            Self::Unavailable => "unavailable",
        }
    }
}

/// The authenticated identity a binding is keyed on. Each variant is
/// authenticated by a different party, and a key of one kind never matches a
/// binding of another: the hub user id `1001` is not the operating-system
/// user `1001`.
...
impl BindingKey {
    fn basis(&self) -> AuthenticationBasis {
        match self {
            Self::OsUser(_) => AuthenticationBasis::OsUser,
            Self::Subject(_) => AuthenticationBasis::OauthSubject,
            Self::EdgeToken(_) => AuthenticationBasis::EdgeToken,
        }
    }

    /// What an unbound identity of this key is called.
    fn default_name(&self) -> String {
        match self {
            Self::OsUser(uid) => format!("os-user:{uid}"),
            Self::Subject(subject) => format!("subject:{subject}"),
            Self::EdgeToken(label) => format!("token:{label}"),
        }
    }

    /// The key as `context_status` reports `authenticated_subject`.
    fn status_form(&self) -> String {
        match self {
            Self::OsUser(uid) => format!("uid:{uid}"),
            Self::Subject(subject) => format!("subject:{subject}"),
            Self::EdgeToken(label) => format!("token:{label}"),
        }
    }
...
impl std::fmt::Display for BindingKey {
    /// The key as a loader refusal or a fail-closed reason names it.
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::OsUser(uid) => write!(f, "OS user {uid}"),
            Self::Subject(subject) => write!(f, "subject {subject:?}"),
            Self::EdgeToken(label) => write!(f, "edge token {label:?}"),
        }
    }
}

impl PrincipalBinding {
    /// The one key this binding selects on, or why it has none: more than one
    /// of the three fields, none of them, or an empty subject or label. The
    /// loader refuses a document on the error, so a binding read from a
    /// loaded policy always has a key.
    pub fn key(&self) -> Result<BindingKey, String> {
        let named: Vec<&str> = [
            ("os_user", self.os_user.is_some()),
            ("subject", self.subject.is_some()),
            ("edge_token", self.edge_token.is_some()),
        ]
        .into_iter()
        ...
        match named.as_slice() {
            ...
            [] => {
                return Err(format!(
                    "principal {:?} names none of os_user, subject and edge_token; a binding names exactly one",
                    self.principal
                ));
            }
            [first, second] => {
                return Err(format!(
                    "principal {:?} names {first} and {second}; a binding names exactly one of os_user, subject and edge_token",
                    self.principal
                ));
            }
            _ => {
                return Err(format!(
                    "principal {:?} names os_user, subject and edge_token; a binding names exactly one",
                    self.principal
                ));
            }
        }
        if let Some(uid) = self.os_user {
            return Ok(BindingKey::OsUser(uid));
        }
        if let Some(subject) = &self.subject {
            if subject.is_empty() {
                return Err(format!(
                    "principal {:?} names an empty subject",
                    self.principal
                ));
            }
            return Ok(BindingKey::Subject(subject.clone()));
        }
        let label = self.edge_token.as_deref().unwrap_or_default();
        if label.is_empty() {
            return Err(format!(
                "principal {:?} names an empty edge_token",
                self.principal
            ));
        }
        Ok(BindingKey::EdgeToken(label.to_owned()))
    }
}
...
pub struct Principal {
    name: String,
    /// The authenticated key bindings are matched on, absent when the basis
...
impl Principal {
    ...
    /// The principal an edge-issued token was issued for, named by the
    /// token's label. The caller has matched the presented token's hash.
    pub fn edge_token(label: &str) -> Self {
        Self::keyed(BindingKey::EdgeToken(label.to_owned()))
    }

    fn keyed(key: BindingKey) -> Self {
        Self {
...
impl PolicyDocument {
    ...
    /// The same document resolved for an identity established elsewhere than
    /// this process: the hosted edge verifies a token and resolves the policy
    /// for the principal it names, never for the service account the edge
    /// runs as.
...
fn parse(encoded: &[u8], source: &Path) -> Result<PolicyFile, String> {
    ...
    // The key each earlier binding passed its check with, so a collision
    // names both halves without re-deriving or trusting a second derivation.
    let mut keys: Vec<BindingKey> = Vec::with_capacity(file.principals.len());
    for (index, binding) in file.principals.iter().enumerate() {
        ...
        let key = binding
            .key()
            .map_err(|reason| format!("{} {reason}", source.display()))?;
        ...
        if let Some(position) = file.principals[..index]
            .iter()
            .position(|earlier| earlier.principal == binding.principal)
        {
            let earlier_key = &keys[position];
            return Err(match (earlier_key, &key) {
                (BindingKey::OsUser(first), BindingKey::OsUser(second)) => format!(
                    "{} declares principal {:?} twice, for OS users {first} and {second}",
                    source.display(),
                    binding.principal,
                ),
                _ => format!(
                    "{} declares principal {:?} twice, for {earlier_key} and {key}",
                    source.display(),
                    binding.principal,
                ),
            });
        }
        if let Some(position) = keys.iter().position(|earlier| earlier == &key) {
            return Err(format!(
                "{} binds {key} to both {:?} and {:?}; one identity cannot hold two principals",
                source.display(),
                file.principals[position].principal,
                binding.principal
            ));
        }
        keys.push(key);
...
impl PolicyDocument {
    ...
    fn resolve_source(&self, cwd: Option<&str>) -> SessionPolicy {
        ...
        // An unauthenticated process has no key to match on, so nothing
        // selects a binding and the fail-closed branch below governs it. A
        // key matches only a binding of its own kind: an OS user never
        // selects a subject binding, whatever the number.
        let binding = principal.key.as_ref().and_then(|key| {
            file.principals
                .iter()
                .find(|binding| binding.key().ok().as_ref() == Some(key))
                .cloned()
        });
        ...
        if let Some(binding) = &binding {
            ...
            fail_closed = Some(match &principal.key {
                Some(BindingKey::OsUser(os_user)) => {
                    format!("effective OS user {os_user} has no principal binding")
                }
                Some(key) => format!("{key} has no principal binding"),
                None => format!(
...
impl SessionPolicy {
    ...
    /// Admit bytes from an identified adapter. Direct fetches have no provider
    /// exception; a provider name in the source's own metadata is not identity.
    pub fn admit_from_provider(
        &self,
    ...
    pub fn mediates_address(&self, url: &str) -> bool {
        if self.private_floor_held {
            return crate::grounding::recordable(url);
        }
    ...
    /// than the policy, never looser, so the policy identity does not move.
    #[must_use]
    pub fn hold_private_floor(mut self) -> Self {
        self.private_floor_held = true;
        self
    }

    /// Whether [`Self::hold_private_floor`] applies to this session.
    pub fn holds_private_floor(&self) -> bool {
        self.private_floor_held
    }
    ...
    pub fn describe(&self) -> Value {
        json!({
            ...
            // Whether the floor is held over that field and the prefixes below:
...
mod tests {
    ...
    // -----------------------------------------------------------------------
    // Token-authenticated principals
    // -----------------------------------------------------------------------

    /// One binding of each key kind, so a test can show that a key selects
    /// only its own kind.
    const TOKEN_BOUND: &str = r#"{"policy_mode":"observe","principals":[{"principal":"research-lead","subject":"hub-user-01","policy_mode":"strict","constraints":[{"kind":"allowed_source_host","host":"allowed.example"}]},{"principal":"copilot-agent","edge_token":"github-copilot"},{"principal":"alice","os_user":1001}]}"#;

    /// The document resolved for a principal established elsewhere than the
    /// process, as the hosted edge resolves it after verifying a token.
    fn policy_as(json: &str, cwd: Option<&str>, principal: Principal) -> SessionPolicy {
        let directory = tempfile::tempdir().expect("tempdir");
        std::fs::write(directory.path().join("policy.json"), json).unwrap();
        PolicyDocument::read(directory.path())
            .expect("valid policy")
    ...
    #[test]
    fn a_verified_subject_selects_its_binding_and_records_its_basis() {
        let policy = policy_as(TOKEN_BOUND, None, Principal::oauth_subject("hub-user-01"));
        assert_eq!(policy.principal(), "research-lead");
        assert_eq!(
            policy.authentication_basis(),
            AuthenticationBasis::OauthSubject
        );
        assert_eq!(policy.mode(), PolicyMode::Strict);
        assert!(!policy.admit_host("https://allowed.example/").is_refusal());
        assert!(policy.admit_host("https://denied.example/").is_refusal());
        ...
        assert_eq!(
            ...
            json!({"name": "research-lead", "basis": "oauth_subject"})
        );
    }

    #[test]
    fn an_edge_token_selects_its_binding_and_records_the_weaker_basis() {
        let policy = policy_as(TOKEN_BOUND, None, Principal::edge_token("github-copilot"));
        assert_eq!(policy.principal(), "copilot-agent");
        assert_eq!(
            policy.authentication_basis(),
            AuthenticationBasis::EdgeToken
        );
        assert_eq!(policy.mode(), PolicyMode::Observe);
        let status = policy.describe();
        assert_eq!(status["authentication_basis"], "edge_token");
        assert_eq!(status["authenticated_subject"], "token:github-copilot");
        assert!(status["fail_closed"].is_null());
    }

    #[test]
    fn an_unbound_token_principal_fails_closed_and_the_reason_names_it() {
        let policy = policy_as(TOKEN_BOUND, None, Principal::oauth_subject("hub-user-99"));
        assert_eq!(policy.principal(), "subject:hub-user-99");
        assert!(policy.admit_host("https://allowed.example/").is_refusal());
        assert_eq!(
            policy.describe()["fail_closed"],
            r#"subject "hub-user-99" has no principal binding"#
        );
        let policy = policy_as(TOKEN_BOUND, None, Principal::edge_token("revoked"));
        assert_eq!(policy.principal(), "token:revoked");
        assert!(policy.admit_host("https://allowed.example/").is_refusal());
        assert_eq!(
            policy.describe()["fail_closed"],
            r#"edge token "revoked" has no principal binding"#
        );
    }

    /// The hub user id `1001` and the operating-system user `1001` are
    /// different identities authenticated by different parties. The digits
    /// match; the bindings must not.
    #[test]
    fn a_key_selects_only_a_binding_of_its_own_kind() {
        let policy = policy_as(TOKEN_BOUND, None, Principal::oauth_subject("1001"));
        assert_eq!(policy.principal(), "subject:1001");
        assert_eq!(
            policy.describe()["fail_closed"],
            r#"subject "1001" has no principal binding"#
        );
        let policy = policy_as(TOKEN_BOUND, None, Principal::edge_token("hub-user-01"));
        assert_eq!(policy.principal(), "token:hub-user-01");
        assert_eq!(
            policy.describe()["fail_closed"],
            r#"edge token "hub-user-01" has no principal binding"#
        );
        let policy = policy_for(TOKEN_BOUND, None, Some(1001));
        assert_eq!(policy.principal(), "alice");
        assert_eq!(policy.authentication_basis(), AuthenticationBasis::OsUser);
        assert_eq!(policy.describe()["authenticated_subject"], "uid:1001");
        assert!(policy.describe()["fail_closed"].is_null());
    }

    /// Ownership, `require_scope` and overlay apply to a token principal by
    /// the rules an OS user meets; only the key that selects the binding
    /// differs.
    #[test]
    fn scope_ownership_and_require_scope_apply_to_token_principals_unchanged() {
        const OWNED: &str = r#"{"principals":[{"principal":"research-lead","subject":"hub-user-01","require_scope":true},{"principal":"copilot-agent","edge_token":"github-copilot"}],"scopes":[{"match":"/work/research","principal":"research-lead","policy_mode":"strict"}]}"#;
        let lead = policy_as(
            OWNED,
            Some("/work/research"),
            Principal::oauth_subject("hub-user-01"),
        );
        assert_eq!(lead.scope(), Some("/work/research"));
        assert_eq!(lead.mode(), PolicyMode::Strict);
        assert!(lead.describe()["fail_closed"].is_null());
        let lead_elsewhere = policy_as(
            OWNED,
            Some("/work/other"),
            Principal::oauth_subject("hub-user-01"),
        );
        assert!(
            lead_elsewhere.describe()["fail_closed"]
                .as_str()
                .unwrap()
                .contains("requires a matching principal-bound directory scope")
        );
        let agent = policy_as(
            OWNED,
            Some("/work/research"),
            Principal::edge_token("github-copilot"),
        );
        assert_eq!(agent.scope(), None);
        assert!(
            agent.describe()["fail_closed"]
                .as_str()
                .unwrap()
                .contains(r#"belongs to principal "research-lead""#)
        );
    }

    #[test]
    fn a_directory_only_policy_resolves_for_a_token_principal_as_for_any_other() {
        const DIRECTORY_ONLY: &str = r#"{"policy_mode":"strict","scopes":[{"match":"ozone","constraints":[{"kind":"allowed_source_host","host":"allowed.example"}]}]}"#;
        let policy = policy_as(
            DIRECTORY_ONLY,
            Some("/work/ozone"),
            Principal::oauth_subject("hub-user-01"),
        );
        assert_eq!(policy.scope(), Some("ozone"));
        assert_eq!(policy.principal(), "subject:hub-user-01");
        assert!(policy.describe()["fail_closed"].is_null());
        assert!(!policy.admit_host("https://allowed.example/").is_refusal());
        assert!(policy.admit_host("https://denied.example/").is_refusal());
    }
    ...
    #[test]
    fn a_binding_names_exactly_one_key_or_is_rejected_at_load() {
        for (json, expected) in [
            (
                r#"{"principals":[{"principal":"alice"}]}"#,
                r#"principal "alice" names none of os_user, subject and edge_token; a binding names exactly one"#,
            ),
            (
                r#"{"principals":[{"principal":"alice","os_user":null,"subject":null}]}"#,
                r#"principal "alice" names none of os_user, subject and edge_token; a binding names exactly one"#,
            ),
            (
                r#"{"principals":[{"principal":"alice","os_user":1001,"subject":"hub-user-01"}]}"#,
                r#"principal "alice" names os_user and subject; a binding names exactly one of os_user, subject and edge_token"#,
            ),
            (
                r#"{"principals":[{"principal":"alice","subject":"hub-user-01","edge_token":"gh"}]}"#,
                r#"principal "alice" names subject and edge_token; a binding names exactly one of os_user, subject and edge_token"#,
            ),
            (
                r#"{"principals":[{"principal":"alice","os_user":1001,"subject":"hub-user-01","edge_token":"gh"}]}"#,
                r#"principal "alice" names os_user, subject and edge_token; a binding names exactly one"#,
            ),
            (
                r#"{"principals":[{"principal":"alice","subject":""}]}"#,
                r#"principal "alice" names an empty subject"#,
            ),
            (
                r#"{"principals":[{"principal":"alice","edge_token":""}]}"#,
                r#"principal "alice" names an empty edge_token"#,
            ),
        ] {
            let error = load_error(json);
            assert!(
                error.ends_with(expected),
                "{json} should be refused with {expected:?}: {error}"
            );
        }
    }

    #[test]
    fn duplicate_token_keys_and_mixed_key_names_are_rejected_at_load() {
        let error = load_error(
            r#"{"principals":[{"principal":"alice","subject":"hub-user-01"},{"principal":"bob","subject":"hub-user-01"}]}"#,
        );
        assert!(
            error.contains(r#"binds subject "hub-user-01" to both "alice" and "bob""#),
            "{error}"
        );
        let error = load_error(
            r#"{"principals":[{"principal":"agent","edge_token":"gh"},{"principal":"other","edge_token":"gh"}]}"#,
        );
        assert!(
            error.contains(r#"binds edge token "gh" to both "agent" and "other""#),
            "{error}"
        );
        let error = load_error(
            r#"{"principals":[{"principal":"alice","os_user":1001},{"principal":"alice","subject":"hub-user-01"}]}"#,
        );
        assert!(
            error.contains(
                r#"declares principal "alice" twice, for OS user 1001 and subject "hub-user-01""#
            ),
            "{error}"
        );
    }
    ...
    fn a_binding_serialises_only_the_key_it_names() {
        let file = parse(
            br#"{"principals":[{"principal":"alice","os_user":1001},{"principal":"lead","subject":"hub-user-01"},{"principal":"agent","edge_token":"gh"}]}"#,
            Path::new("policy.json"),
        )
        .expect("a valid policy");
        let value = serde_json::to_value(&file).expect("serialises");
        assert_eq!(
            value["principals"][0],
            json!({"principal": "alice", "os_user": 1001, "require_scope": false})
        );
        assert_eq!(
            value["principals"][1],
            json!({"principal": "lead", "subject": "hub-user-01", "require_scope": false})
        );
        assert_eq!(
            value["principals"][2],
            json!({"principal": "agent", "edge_token": "gh", "require_scope": false})
        );
    }
    ...
    fn existing_policy_identity_vectors_digest_unchanged() {
        ...
        const OWNED: &str = r#"{"policy_mode":"observe","principals":[{"principal":"alice","os_user":1001,"require_scope":true,"policy_mode":"strict"}],"scopes":[{"match":"alice-work","principal":"alice","constraints":[{"kind":"allowed_source_host","host":"allowed.example"}]}]}"#;
        const DIRECTORY_ONLY: &str = r#"{"policy_mode":"strict","scopes":[{"match":"ozone","constraints":[{"kind":"allowed_source_host","host":"allowed.example"}]}]}"#;
        for (name, json, cwd, uid, expected) in [
            (
                "bound-alice",
                BOUND,
                None,
            ...
            assert_eq!(policy.identity().schema, IDENTITY_SCHEMA);
            assert_eq!(policy.identity().resolver, RESOLVER_VERSION);
            assert_eq!(policy.identity().digest, expected, "vector {name} moved");
        }
    }
    ...
    /// Service mode holds the floor over both fields that lift it: the cloud
    /// metadata addresses and every private range stay refused under a
    /// policy that admits them, and the identity does not move.
    #[test]
    fn a_held_floor_refuses_private_addresses_whatever_the_policy_says() {
        let text = r#"{"policy_mode":"observe","allow_private_hosts":true,
            "record_internal_prefixes":["http://169.254.169.254/","http://metadata.google.internal/"]}"#;
        let admitting = policy(text);
        let held = policy(text).hold_private_floor();
        assert!(held.holds_private_floor() && !admitting.holds_private_floor());
        for private in [
            ...
            assert!(admitting.mediates_address(private), "{private}");
            assert!(!held.mediates_address(private), "{private}");
        }
        assert!(held.mediates_address("https://www.gov.uk/x"));
        assert_eq!(held.describe()["private_floor"], "held");
        assert_eq!(admitting.describe()["private_floor"], "policy");
        assert_eq!(held.identity(), admitting.identity());
    }
    ...
    fn the_edge_and_hub_digest_one_policy_equally() {
        ...
        const OWNER_FORM: &str = r#"{"policy_mode":"strict","constraints":[{"kind":"access_rule","host":"www.legislation.gov.uk","action":"allow"},{"kind":"access_rule","host":"www.gov.uk","action":"allow"},{"kind":"access_rule","host":"research.example.com","action":"require_licence","licence":"example-firm/research-subscription-2026"},{"kind":"access_rule","host":"*.theguardian.com","action":"allow"},{"kind":"access_rule","host":"*","action":"refuse"}],"scopes":[{"match":"/matters/confidential-","engagement":"client-confidential","allow_telemetry_egress":false,"constraints":[{"kind":"access_rule","host":"*","action":"refuse"}]},{"match":"/matters/M-2026-014","engagement":"M-2026-014","allow_telemetry_egress":true}]}"#;
        let owner_form: Value = serde_json::from_str(OWNER_FORM).expect("the vector is json");
        assert_eq!(
            canonical_digest(&owner_form),
            "sha256:abc7ec40353303303746a27a7dac248eacd1a83720eb9dd5a6cc584af82dd77d"
        );
        ...
        assert_eq!(
            ...
            "sha256:1e938458efd3b653542d0ac8e0288d8647b11b7e994c2949c9fc3bd0212191c5",
