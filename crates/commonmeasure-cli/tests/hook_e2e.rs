...
fn boundaries_name_the_policy_identity_or_why_it_is_unavailable_and_older_records_are_said_to_predate_it()
 {
    ...
    // The boundaries, without the host-process record session start also
    // writes, which names no policy.
    let recorded: Vec<Value> = records(home.path(), "s-id")
        .into_iter()
        .filter(|record| record["event"] != "host_process")
        .collect();
...
/// The five hooks `install claude` registers, each with a payload in Claude
/// Code's own shape and nothing another host's shape adds.
fn claude_shaped_events(session: &str) -> [(&'static str, Value); 5] {
    [
        (
            "session-start",
        ...
        (
            "session-end",
            json!({"session_id": session, "hook_event_name": "SessionEnd", "cwd": "/work",
...
/// The session-start hook records the host process it runs under, which
/// here is this test process: the hook's parent, and not a shell. The pair
/// is the pid and the start time to the second; the command is a name and
/// never an argument. The session-end hook records the host's reason word
/// as given, and a payload without one leaves the field absent.
#[test]
fn session_start_records_the_host_process_and_session_end_the_hosts_reason() {
    let home = tempfile::tempdir().expect("tempdir");
    let output = hook(
        home.path(),
        "session-start",
        json!({"session_id": "s-host", "hook_event_name": "SessionStart",
               "cwd": "/work", "source": "startup"}),
    );
    assert!(output.status.success());
    let written = records(home.path(), "s-host");
    let record = written
        .iter()
        .find(|record| record["event"] == "host_process")
        .unwrap_or_else(|| panic!("no host_process record: {written:?}"));
    let payload = &record["payload"];
    assert_eq!(payload["session_id"], "s-host");
    assert_eq!(payload["host"], "claude-code");
    assert_eq!(payload["path"], "hook");
    assert!(payload["writer_pid"].as_u64().is_some_and(|pid| pid > 1));
    assert!(
        payload["basis"]
            .as_str()
            .is_some_and(|basis| basis.contains("process table"))
    );
    if cfg!(any(target_os = "macos", target_os = "linux")) {
        assert_eq!(
            payload["host_process"]["pid"],
            std::process::id(),
            "the hook's parent is this test process: {payload}"
        );
        let started_at = payload["host_process"]["started_at"].as_str().unwrap();
        assert!(
            started_at.len() == 20 && started_at.ends_with('Z'),
            "to the second, in UTC: {started_at}"
        );
        let command = payload["host_process"]["command"].as_str().unwrap();
        assert!(
            command.starts_with("hook_e2e") && !command.contains(' ') && !command.contains('/'),
            "an executable name and nothing else: {command}"
        );
        assert!(payload.get("unavailable").is_none(), "{payload}");
    } else {
    ...
    let output = hook(
        home.path(),
        "session-end",
        json!({"session_id": "s-host", "hook_event_name": "SessionEnd",
               "cwd": "/work", "reason": "clear"}),
    );
    assert!(output.status.success());
    assert!(output.stdout.is_empty(), "session end prints nothing");
    let output = hook(
        home.path(),
        "session-end",
        json!({"session_id": "s-host", "hook_event_name": "SessionEnd", "cwd": "/work"}),
    );
    assert!(output.status.success());
    let ended: Vec<Value> = records(home.path(), "s-host")
        .into_iter()
        .filter(|record| record["event"] == "session_ended")
        .collect();
    assert_eq!(ended.len(), 2, "{ended:?}");
    assert_eq!(ended[0]["payload"]["reason"], "clear");
    assert_eq!(ended[0]["payload"]["host"], "claude-code");
    assert!(ended[1]["payload"].get("reason").is_none(), "{}", ended[1]);

    // Another host's payload under Claude Code's event name is refused at
    // session end as it is at the other four.
    let output = hook(
        home.path(),
        "session-end",
        json!({"session_id": "s-vscode", "hook_event_name": "SessionEnd", "cwd": "/work",
               "timestamp": "2026-09-17T22:55:48Z", "reason": "other"}),
    );
    assert!(output.status.success());
    assert!(records(home.path(), "s-vscode").is_empty());
}
...
fn hooks_append_after_a_truncated_record_and_concurrent_hooks_both_record() {
    ...
    let path = sessions.join("damaged.ndjson");
    let prefix = b"{\"event\":\"existing\"}\n{\"partial\":";
    std::fs::write(&path, prefix).unwrap();
    let payload = |session: &str, url: &str| {
        json!({
            "session_id":session, "hook_event_name":"PostToolUse", "tool_name":"WebFetch",
            "tool_input":{"url":url}, "tool_response":{"result":"synthetic text"}
        })
    };
    let output = hook(
        home.path(),
    ...
    std::thread::scope(|scope| {
        for url in ["https://example.com/a", "https://example.com/b"] {
            let barrier = &barrier;
            let home = home.path();
            let payload = payload("racing", url);
            scope.spawn(move || {
                barrier.wait();
                assert!(hook(home, "post-tool-use", payload).status.success());
            });
        }
    });
    let rows = records(home.path(), "racing");
    assert_eq!(rows.len(), 2);
    assert!(rows.iter().all(|row| row["event"] == "crossing_observed"));
    assert_ne!(rows[0]["payload"]["url"], rows[1]["payload"]["url"]);
}

/// Catches: a local edge that delivers only when a person runs `commonmeasure
/// relay`, and a session-end hook that waits on the delivery it starts. The
/// receiver holds its answer until the hook has returned, so the hook exiting
/// zero first shows it did not wait; the batch then arriving shows the relay
/// ran without anyone asking. The receiver is a loopback server recording the
/// bytes posted, not a hub.
#[test]
fn a_session_end_relays_in_the_background_without_the_hook_waiting() {
    ...
    for (event, payload) in [
        (
            "user-prompt-submit",
            json!({"session_id": session, "hook_event_name": "UserPromptSubmit",
                   "prompt_id": "p1", "cwd": "/work/personal", "prompt": "the cap?"}),
        ),
        (
            "post-tool-use",
            json!({"session_id": session, "hook_event_name": "PostToolUse",
                   "prompt_id": "p1", "cwd": "/work/personal", "tool_name": "WebFetch",
                   "tool_input": {"url": "https://www.ofgem.gov.uk/cap"},
                   "tool_response": {"result": "The cap is set quarterly."}}),
        ),
        (
            ...
            json!({"session_id": session, "hook_event_name": "Stop",
                   "prompt_id": "p1", "cwd": "/work/personal"}),
        ),
    ] {
        assert!(hook(home.path(), event, payload).status.success());
    }
    assert!(
        bodies.lock().unwrap().is_empty(),
        "no hook but session end relays"
    );
    ...
    assert!(ended.stdout.is_empty() && ended.stderr.is_empty());
    assert!(
        bodies.lock().unwrap().is_empty(),
        "the hook returned before the receiver answered"
    );
    released.store(true, Ordering::SeqCst);

    let deadline = std::time::Instant::now() + std::time::Duration::from_secs(30);
    while bodies.lock().unwrap().is_empty() && std::time::Instant::now() < deadline {
        std::thread::sleep(std::time::Duration::from_millis(50));
    }
    // The relay holds the spool lock until it has recorded the acceptance
    // and closed the spool; taking the lock means it has finished.
    let lock = home.path().join("relay/spool/delivery.lock");
    loop {
        let finished = std::fs::File::open(&lock)
            .ok()
            .is_some_and(|file| file.try_lock().is_ok());
    ...
    assert_eq!(bodies.len(), 1, "one batch for the one session");
    let mut types: Vec<&str> = bodies[0]["events"]
        .as_array()
        .unwrap()
        .iter()
    ...
    let receipts: Value = serde_json::from_slice(
        &std::fs::read(home.path().join("relay/receipts.json")).expect("the run left receipts"),
    )
    .unwrap();
    assert!(receipts["last_delivered_at"].is_string(), "{receipts}");

    // `doctor` states the delivery and its age, and that relaying is
    // automatic. A host home of its own keeps the operator's host files out.
    let hosts = tempfile::tempdir().expect("tempdir");
    let doctor = Command::new(env!("CARGO_BIN_EXE_commonmeasure"))
        .arg("doctor")
        .env("COMMONMEASURE_HOME", home.path())
        .env("HOME", hosts.path())
        ...
        .output()
        .expect("doctor runs");
    let printed = String::from_utf8_lossy(&doctor.stdout);
    let delivered_at = receipts["last_delivered_at"].as_str().unwrap();
    // The receiver is the configured one, which the egress lines above
    // already name, so the delivery line states the time and its age alone.
    assert!(
        printed.contains(&format!("last delivery: {delivered_at}, ")) && printed.contains(" ago\n"),
        "{printed}"
    );
    // This host home holds no Claude Code registration, so nothing here
    // would send the event the hook was driven with by hand.
    assert!(
        printed.contains("automatic relay: off, no Claude Code registration sends SessionEnd"),
        "{printed}"
    );
    std::fs::create_dir_all(hosts.path().join(".claude")).unwrap();
    std::fs::write(
        hosts.path().join(".claude/settings.json"),
        json!({"hooks": {"SessionEnd": [{"hooks": [
            {"type": "command", "command": "/opt/commonmeasure hook session-end"}
        ]}]}})
        .to_string(),
    )
    .unwrap();
    let registered = Command::new(env!("CARGO_BIN_EXE_commonmeasure"))
        .arg("doctor")
        .env("COMMONMEASURE_HOME", home.path())
        .env("HOME", hosts.path())
        .env_remove("CLAUDE_CONFIG_DIR")
        ...
        .output()
        .expect("doctor runs");
    let printed = String::from_utf8_lossy(&registered.stdout);
    assert!(
        printed.contains("automatic relay: at each Claude Code session end"),
        "{printed}"
    );

    // The marker outranks the registration, and the line says what else it
    // does: a source demanding usage reporting is refused while it is there.
    std::fs::write(home.path().join("relay/manual"), b"").unwrap();
    let manual = Command::new(env!("CARGO_BIN_EXE_commonmeasure"))
        .arg("doctor")
        .env("COMMONMEASURE_HOME", home.path())
        .env("HOME", hosts.path())
        .env_remove("CLAUDE_CONFIG_DIR")
        ...
        .output()
        .expect("doctor runs");
    let printed = String::from_utf8_lossy(&manual.stdout);
    let line = printed
        .lines()
        .find(|line| line.contains("automatic relay:"))
        .unwrap_or_else(|| panic!("an automatic relay line: {printed}"));
    assert!(
        line.contains("automatic relay: off, the marker file")
            && line.contains("relay/manual")
            && line.contains("a source whose licence demands usage reporting is refused while the marker is there"),
        "{line}"
    );
}

/// Catches: a session end starting a relay where the operator configured no
/// receiver, or wrote the `relay/manual` marker to review each run first. In
/// both the hook records the end and starts nothing, so no spool, no relay
/// state and no delivery appear (owner decision, 22 September 2026).
#[test]
fn a_session_end_starts_nothing_without_a_receiver_or_under_the_manual_marker() {
    use std::sync::{Arc, Mutex};
    ...
    for manual in [false, true] {
        ...
        assert!(ended.status.success(), "{manual}");
        // A started relay would create the spool and the delivery state
        // before it posted anything, so their absence is the whole check.
        std::thread::sleep(std::time::Duration::from_millis(300));
        assert!(
            !home.path().join("relay/spool").exists(),
            "manual={manual}: a relay run would have created the spool"
        );
        assert!(
            !home.path().join("relay/receipts.json").exists(),
            "manual={manual}: a relay run would have written receipts"
        );
        assert!(
            bodies.lock().unwrap().is_empty(),
            "manual={manual}: nothing was posted"
        );
        assert_eq!(
            std::fs::read_to_string(home.path().join(format!("sessions/{session}.ndjson")))
                .unwrap()
                .matches("session_ended")
                .count(),
            1,
            "manual={manual}: the end is still recorded"
        );
    }
    receiver.stop();
}
