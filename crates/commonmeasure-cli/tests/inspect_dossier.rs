...
fn every_citation_in_the_replay_dossier_resolves_to_a_record() {
    let run = &evidence().join("output/replay");
...
fn every_citation_in_the_gateway_dossier_resolves_to_a_record() {
    let run = &evidence().join("output/tensorzero");
    assert_every_citation_resolves(run, &dossier(run));
}
...
fn every_citation_in_the_skill_dossier_resolves_to_a_record() {
    let run = &evidence().join("output/skills");
    assert_every_citation_resolves(run, &dossier(run));
}
...
fn replayed_bytes_are_declared_as_recordings_with_their_provenance() {
    let text = flat(&dossier(&evidence().join("output/replay")));
...
fn a_local_acquisition_is_described_as_capture_not_replay() {
    let text = flat(&dossier(&evidence().join("output/tensorzero")));
...
fn a_plan_with_a_null_evaluation_renders_absence_not_a_zero_score() {
    ...
    copy_run(&evidence().join("output/replay"), &run);
...
fn an_unmeasured_freshness_section_still_lists_its_dated_parts() {
    let directory = tempfile::tempdir().expect("tempdir");
    let run = directory.path().join("run");
    copy_run(&evidence().join("output/latest"), &run);
...
fn the_contract_line_states_the_schema_version_and_flags_a_stale_one() {
    let current = flat(&dossier(&evidence().join("output/latest")));
    assert!(
    ...
    let stale = flat(&dossier(&evidence().join("output/replay")));
    assert!(
...
fn cited_sealed_responses() -> Option<PathBuf> {
    let responses = evidence().join("output/cited/responses");
    if responses.is_dir() {
        return Some(responses);
    }
    eprintln!(
        "skipping: output/cited/responses is not present in the fixture directory — sealed \
...
fn a_published_supported_span_is_rechecked_against_the_sealed_response() {
    ...
    let run = &evidence().join("output/cited");
    let text = flat(&dossier(run));
    assert!(
...
fn a_tampered_supported_span_is_a_mismatch_not_provenance() {
    ...
    let directory = tempfile::tempdir().expect("tempdir");
    let run = directory.path().join("run");
    copy_run(&evidence().join("output/cited"), &run);
...
fn the_cited_dossier_carries_the_fidelity_verdicts_and_the_judge_agreement() {
    let text = flat(&dossier(&evidence().join("output/cited")));
    assert!(
...
fn the_manifest_line_names_exactly_what_the_manifest_seals() {
    // The fixture set is exactly the committed runs. `output/live` is
    ...
    for (run, recipe) in [
        ("output/replay", "just replay-example"),
        ("output/latest", "just rubric-example"),
        ("output/cited", "just cited-example"),
        ("output/tensorzero", "just internal-corpus-example"),
    ] {
        let run = &evidence().join(run);
        assert!(
    ...
    let current = named_sealed_inputs(&dossier(&evidence().join("output/latest")));
    assert!(
...
fn the_licensed_replay_dossier_shows_the_quote_before_the_purchase_decision() {
    let text = flat(&dossier(&evidence().join("output/redpine-replay")));
    let quote = text
