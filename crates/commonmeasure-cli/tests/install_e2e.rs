...
fn after_has_no_session_end(settings: &std::path::Path) -> bool {
    json_at(settings)["hooks"].get("SessionEnd").is_none()
}

/// `install claude` writes the five hooks and the MCP server, each naming
...
fn claude_registration_round_trip_touches_only_this_products_entries() {
    ...
    assert!(
        text.contains("five hooks (SessionStart, PostToolUse, UserPromptSubmit, Stop, SessionEnd)"),
        "{text}"
    );
    assert!(
    ...
    assert_eq!(
        ...
        format!(
            "\"{}\" hook session-end --host claude-code",
            binary.display()
        ),
        "the fifth hook reports the session ending"
    );
    assert_eq!(
        written["hooks"].as_object().unwrap().len(),
        5,
        "five events carry a hook of this product"
    );
    ...
    assert!(
        text.contains(
            "hooks: SessionStart, PostToolUse, UserPromptSubmit, Stop, SessionEnd registered"
        ),
        "{text}"
    );
    assert!(
    ...
    assert!(output.status.success(), "{}", stderr(&output));
    let text = stdout(&output);
    assert!(text.contains("5 hook handler(s) removed"), "{text}");
    assert!(after_has_no_session_end(&settings), "SessionEnd is removed");
