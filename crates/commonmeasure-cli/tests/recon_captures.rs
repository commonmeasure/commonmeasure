...
//!
//! `recon/` holds recorded provider exchanges whose NOTES declare two
...
/// The elision convention the captures declare (`recon/redpine/NOTES.md`):
...
fn capture_files(dir: &Path, found: &mut Vec<PathBuf>) {
    let entries = std::fs::read_dir(dir).expect("recon/ is readable");
...
fn no_capture_carries_a_session_or_credential_value() {
    ...
    capture_files(&evidence().join("recon"), &mut files);
    assert!(
        files.len() >= 10,
        "the sweep found {} captures; recon/ has moved and this test is scanning nothing",
        files.len()
    );
    for file in &files {
...
fn licensed_mention_text_is_truncated_in_every_carrier() {
    let mut files = Vec::new();
    capture_files(&evidence().join("recon/redpine"), &mut files);
