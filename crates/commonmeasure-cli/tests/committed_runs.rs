...
/// The committed run directories under `output/` of the fixture directory.
fn committed_runs(evidence: &Path) -> Vec<PathBuf> {
    let mut found = Vec::new();
    collect(&evidence.join("output"), &mut found);
    found.sort();
    assert!(
        found.len() > 50,
...
fn collect(dir: &Path, found: &mut Vec<PathBuf>) {
    // `.gitignore` keeps `output/live-*/` out of the fixture repository, so a
...
fn a_committed_run_declaring_a_current_version_carries_that_version_s_keys() {
    ...
    let manifest_version = {
        let latest = read(&evidence.join("output/latest/manifest.json"));
    ...
    for run in committed_runs(&evidence) {
        let relative = run
            .strip_prefix(&evidence)
            .expect("a run under the fixture directory")
            .to_string_lossy()
            .into_owned();
...
fn the_committed_example_carries_the_current_shape() {
    let latest = evidence().join("output/latest");
