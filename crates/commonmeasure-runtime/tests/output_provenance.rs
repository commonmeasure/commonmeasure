...
#[test]
fn explicit_trust_is_isolated_between_readers_in_the_same_process() {
    let run = Run::execute(&suite(true), demonstration_identity());
    ...
    let trusted = provenance::read_back_with_trust(&text, Some(root)).unwrap();
    assert_eq!(trusted.validation["state"], "trusted");
    let default = provenance::read_back(&text).unwrap();
    assert_eq!(default.validation["state"], "valid");
    assert_eq!(default.validation["trusted"], false);
    assert!(default.validation["trust_anchors_hash"].is_null());
}

#[test]
fn a_remote_suite_seals_network_permission_and_never_falls_back_to_local_signing() {
    let mut suite = suite(true);
    suite.output_provenance.as_mut().unwrap().encypher = Some(provenance::encypher::Disclosure {
        send_answer_and_source_record: true,
    });
    let run = Run::execute(&suite, demonstration_identity());
    let plan = run.plan("internal-only");
    let invocation = run.provenance_invocations(plan)[0];
    assert_eq!(invocation["decision"], "abstain");
    assert_eq!(invocation["detail"]["remote"]["request_attempted"], false);
    assert!(invocation["outputs"].as_array().unwrap().is_empty());
    assert!(!run.output.join("provenance/internal-only.txt").exists());
    let sealed: Value =
        serde_json::from_slice(&std::fs::read(run.output.join("manifest.json")).unwrap()).unwrap();
    let processor = sealed["manifest"]["processors"]
        .as_array()
        .unwrap()
        .iter()
        .find(|p| p["name"] == provenance::NAME)
        .unwrap();
    assert_eq!(processor["permissions"]["network"], true);
    assert_eq!(
        processor["configuration_digest"],
        invocation["processor"]["configuration_digest"]
    );
}
