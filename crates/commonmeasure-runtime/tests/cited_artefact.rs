//! The published cited run, witnessed where it stands: `demo/output/cited`
...
fn a_published_supported_span_excises_from_the_retained_text_to_its_quote() {
    ...
    if !run_dir().join("responses").is_dir() {
        eprintln!(
            "skipping the excision half: demo/output/cited/responses is not present in this \
