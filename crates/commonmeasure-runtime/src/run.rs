...
/// period of them (`DECISIONS.md` §Delegated authority and fleet management).
...
/// the adapter, the transport, the parser or the policy — the only substitution
/// `AGENTS.md` allows, and the seam recorded replay enters through
/// (`docs/contracts/run-output.md` §Replay).
///
...
impl Execution<'_> {
    ...
    fn provider_plan(&mut self, supply_plan: &SupplyPlan) -> Result<Value, RunError> {
        ...
        let admissions: Vec<Ruling> = acquisition
            .envelopes
            .iter()
            .map(|envelope| policy::source_admission(&self.suite.job, envelope))
            .collect();
    ...
    fn quoted_purchase(
        ...
        // purchase, the other the period's total (`DECISIONS.md` §Delegated authority and fleet management).
...
fn manifest(
    ...
    let mut manifest = json!({
        ...
        "processors": processor::installed(),
