//! The HTML text extractor with credential verification before transformation.
//!
...
Content-Type, is decoded under the rules below without HTML extraction.\n\
credentials: before lossy decoding or extraction, verify supported UTF-8 text credentials against \
the body after removal of content coding, using the C2PA SDK offline with no trust list. \
For HTML, read Annex A.7 inline manifests and record external links as unavailable without \
fetching them. For other text/* media types, read Annex A.8; remove a recovered wrapper \
after verification, including when the manifest is invalid or untrusted. An unsupported \
or incomplete wrapper stays in the text. Other media types and non-UTF-8 bodies have \
unavailable verification. Record absent, invalid, untrusted and unavailable separately; \
no configured trust list means no trusted result.\n\
...
static MANIFEST: LazyLock<ProcessorManifest> = LazyLock::new(|| ProcessorManifest {
    ...
    // Text transformation is deterministic; certificate validation can
    // depend on the verification time.
    determinism: Determinism::NonDeterministic,
    limits: "in-process and synchronous; offline credential verification, no supervisor timeout",
    failure_by_mode: FailureByMode {
        // Credential verification is evidence, not an admission rule.
        // Unavailable verification leaves a gap; the screens still decide
        // whether the transformed text may enter context.
...
pub fn invoke(
    ...
    let inspection = super::embedded::inspect(body, content_type);
    let wrapper_removed = inspection.clean_text.is_some();
    ...
    let mut removed = coded
        .map(|(coding, _)| format!("the {coding} content coding removed, "))
        .unwrap_or_default();
    if wrapper_removed {
        removed.push_str("the Annex A.8 credential wrapper removed after verification, ");
    }
    ...
    let gaps = if inspection.evidence["state"] == "unavailable" {
        vec![Gap::new(
            GapReason::CapabilityUnavailable,
            inspection.evidence["reason"].as_str().unwrap_or_default(),
        )]
    } else {
        Vec::new()
    };
