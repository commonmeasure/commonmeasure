...
//!
//! The signing identity is the operator's. Without a certificate the
...
//! claim is inspectable even when it could not be attested. No trust list is
//! configured, so a well-formed signature validates as valid but untrusted,
...
pub fn read_back(text: &str) -> Result<ReadBack, ReadBackError> {
    ...
    let (state, trusted, summary) = match reader.validation_state() {
        c2pa::ValidationState::Trusted => (
            "trusted",
            true,
            "valid and trusted: the signature verifies and the certificate chains to a \
             trust list",
        ),
        c2pa::ValidationState::Valid if untrusted_only => (
            "valid",
            false,
            "valid but untrusted: the signature verifies and the certificate is on no \
             trust list",
        ),
