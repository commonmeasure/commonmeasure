...
This directory is the routing experiment's job population (`docs/READ-A-HOLDOUT.md`): the
...
carries the registration date and digest (`docs/READ-A-HOLDOUT.md` §0). Terms like
...
Frozen after the 24 fitting runs
(`demo/output/commerce-routing/`) and before any holdout run. The rule is
...
above, and `docs/READ-A-HOLDOUT.md` reads the whole experiment from the
committed records: the registration's integrity, the rule re-derived, the
comparison and the loss mechanism.
