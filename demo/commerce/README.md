...
The corpus holds sixteen documents. The committed comparison runs under
`demo/output/commerce/` sealed a nine-document subset by manifest hash;
...
## The committed runs — `demo/output/commerce/`

Regenerated with the gateway up:
...
The walkthrough is `docs/READ-A-COMPARISON.md`: what the four conditions
measured per question, where the mixed-supply claim holds and where it
does not, and how to check every figure against the artefacts.
