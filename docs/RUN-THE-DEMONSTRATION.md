...
leave the machine. A batch run in which the same malicious page is blocked
is read in the terminal, not the console. It runs offline over the local
corpus in `demo/injection/corpus/`:

```sh
COMMONMEASURE_INTERNAL_CORPUS=demo/injection/corpus cargo run -q -p commonmeasure-cli -- \
  run demo/jobs/injection-screen.json --output target/demo/injection
cargo run -q -p commonmeasure-cli -- inspect target/demo/injection
```
...
the batch run's report. What they cannot reproduce: live purchasing from
