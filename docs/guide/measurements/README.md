...
Reads committed files from this repository — prose, Rust, JSON Schema, JSON
test vectors, `Cargo.lock` — and reports tokens per 1,000 characters for each. The
guide's table was measured on 4 August 2026 over an earlier set of files, four
of which are not published; a rerun over the current set gives comparable
rather than identical figures.
