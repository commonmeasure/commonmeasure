...
machine and at every redirect hop. In `strict` mode, rules allowing only named hosts
create a closed read list for that path. The default `observe` mode records a breach
and allows the fetch. A robots.txt `Disallow` in the product token's group—or in `*`
when there is no such group—also refuses in `strict` mode and is recorded in every
mode.
...
There is also a reporting limitation in the robots.txt account: the full record
carries the requested host's robots.txt URL, but the breach text and agent-facing
summary give only the group. That can obscure which host refused access, especially
through a link shortener.[^implementation-robots] The useful operational record
therefore includes both the detailed decision and a clear account of coverage:
which inputs were controlled, which were only observed and which were unavailable.
...
[^implementation-robots]: A link-shortener request on 14 September 2026 was
    ...
    site allowed the fetch. robots.txt is read for the requested host; the full
    record carries its URL, while the breach and agent-facing summary give only
    the group.
<!-- product-only:end -->
