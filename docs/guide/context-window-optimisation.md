...
logs and generated files. The table below measures nine files from this
project with one named tokeniser, so it provides a starting point rather
than a universal conversion factor.

| Content | Tokens per 1,000 characters | Characters per token |
|---|---|---|
| English prose (a product definition document) | 191 | 5.22 |
| English prose, longer (a delivery roadmap) | 224 | 4.47 |
...
`docs/guide/measurements/token_density.py`. The two prose documents, the JSON
record and the event log measured then are not published; the script now
reads published files in their place, with JSON test vectors standing in for
the event log and no JSON API record, so a rerun gives comparable rather than
identical figures. They do not give exact Claude or
...
Preserve applicable source conditions as well as answer evidence. Extraction,
selection and compaction must retain attribution, permitted-use and reporting
references for material still in use. These can live in structured records
enforced outside the prompt, provided later actions can still resolve and honour
them. A shorter prompt must not silently remove a condition or turn a source's
reporting request into permission to disclose private data. The companion guide
explains [how publisher conditions differ from instruction
authority](untrusted-context.md#publisher-instructions-and-attacks). Establish
policy eligibility before ranking; a higher usefulness score cannot override it.
...
- Token density used tiktoken `o200k_base` across nine files from this
  project. Four of them are not published; the measurement script reruns the
  method on published files of the same types.
