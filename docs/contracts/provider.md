...
- `query` — retrieve directly from a bounded corpus. Implemented by the
  `internal` adapter over a directory of documents the operator owns; an
  operator's RAG endpoint is configuration on the same shape, not new
  architecture. Because the corpus is bounded, a shortfall against the
...
the operator's access, and what that leaves open is recorded in
...
Nothing verifies a declared date against when the content was written
([`docs/knowledge-base/unverified-assumptions.md`](../knowledge-base/unverified-assumptions.md)). An envelope with no
...
read. An adapter's verification state (below) is recorded in
[`docs/knowledge-base/provider-verification.md`](../knowledge-base/provider-verification.md) and is `spec-verified` at
most until a dated live call's response bytes are committed under
`demo/recon/`.
