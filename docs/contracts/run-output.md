...
run, plan, manifest and evidence log are defined in [`docs/GLOSSARY.md`](../GLOSSARY.md). The
committed example is `demo/output/latest/`: a replay of the committed
recordings with no credentials and no gateway configured, ranked against the
job's declared coverage rubric. The committed replay example with answers is
`demo/output/replay/`. [`docs/contracts/session-evidence.md`](session-evidence.md) is the matching
...
```sh
cargo run -p commonmeasure-cli -- run demo/jobs/eu-ai-act-replay-rubric.json \
  --replay demo/recon --output demo/output/latest
...
manifest hash across artefacts. [`docs/READ-A-RUN.md`](../READ-A-RUN.md) is the five-minute
walkthrough; `crates/commonmeasure-cli/tests/inspect_dossier.rs` checks that every
printed citation resolves.

A replay run serves committed recorded provider responses instead of calling
any provider:

```sh
cargo run -p commonmeasure-cli -- run demo/jobs/eu-ai-act-replay.json \
  --replay demo/recon --output demo/output/replay
```
...
Two dependencies are external and neither is assumed:
...
Where either is absent the affected plan is `unavailable` and carries a gap
naming what is missing ([`docs/FAIL-POLICY.md`](../FAIL-POLICY.md) §5).

The committed skill run is `demo/output/skills/`, produced by:
...
COMMONMEASURE_SKILL_CATALOGUE=demo/skills/catalogue.json \
  ...
  --live --output demo/output/skills
```
...
`responses/` is `.gitignore`d: it holds unredacted provider bodies, which may
contain licensed content. The committed replay examples are the exception:
their sealed responses are copies of the committed, redacted recon captures,
so `demo/output/replay/responses/`, `demo/output/cited/responses/` and
`demo/output/latest/responses/` are retained. `demo/output/skills/responses/`
is retained because nothing there came from a provider: it is the output of
two local programs run over a bundle committed in this repository, by
children spawned with an empty environment.
...
from a verified recording. Every other outcome is `planned`. The standing
per-provider record is [`docs/knowledge-base/provider-verification.md`](../knowledge-base/provider-verification.md); a
plan's state is evidence about this run only.
...
volatile fields. This contract, the runtime and `demo/output/latest/`
change together.
...
- Skill invocation is a supervised spawn, not a sandbox. The child runs as
  ...
  result whole. What remains open is recorded in
  [`docs/knowledge-base/unverified-assumptions.md`](../knowledge-base/unverified-assumptions.md) §Skills.
