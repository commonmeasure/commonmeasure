...
Common Measure Ltd runs one at hub.commonmeasure.ai. An edge is complete without
it for local acquisition: managed source policy is enforced on the machine,
and an unreachable Hub never relaxes it. Planned organisational entitlement
issuance or validation may require an online Hub service for the selected supplier
route; cached policy does not renew an expired entitlement. This guide
describes current managed-policy behaviour. What it
...
Each organisation has its own tenant within that shared Hub service. “The
organisation's Hub” means its members, policy, enrolled edges and cleared
records; it does not mean a separate server. An organisation can enrol several
local edges and use a hosted Edge for remote integrations such as Word.

Edge enrolment registers the runtime's key with the organisation. Signing in
from Word authorises a person to call the organisation's existing hosted Edge;
it does not enrol their computer or create another Edge. Several host
registrations can also share one local Edge home and enrolment.

In this guide, the machine holding the private record is the machine running
Edge. For a hosted Edge that is the service's storage, not the user's laptop.
The service operator processes acquired content there; Hub's telemetry boundary
still excludes that content.
...
Cleared turn boundaries carry their recorded turn ID when present, their
timestamp and a `minimal` privacy declaration. Their directory and transcript
path stay private. Coverage is selected under the
[projection contract](contracts/session-evidence.md#content-telemetry-projection).
