...
1. **version** builds the binary, fails when the tag is not `v` followed by
   ...
   the notes file for the tag is missing. Nothing is built for release until
...
3. **publish** packages the plugin archive from those binaries with
   ...
   release under the tag, titled by the notes file's first heading and
   bodied by the rest of it.
