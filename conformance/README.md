...
others do not (retrieval, grounding and minimal turn boundaries, a derived
session id, an enrolled edge's
...
a session that was refused twice and admitted once). `session-turns.json`
adds recorded turn IDs and `turn.privacy_level: minimal`, with no source URL
on a turn event. `session-registered.json` is a session registered part-way
through and renewed once, projected for the hub that issued the instance: its
content events carry the event-level `instance` member at each record's
revision, and the crossing before the registration carries none
(`docs/contracts/session-evidence.md` §Instance reference). Every event
declares selected coverage and Grounding level in
`data.commonmeasure-projection`; the selection rule is
`docs/contracts/session-evidence.md` §Selected coverage. Run ingestion counts
carry their recorded `token_basis` beside `tokens_ingested`, as session counts
already do. Behavioural
