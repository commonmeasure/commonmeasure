...
both: each carries the same five hooks, and two registrations record every
...
soon as you work. (The same file declares two more hooks: `SessionStart`,
which is delivery rather than capture — §The standing nudge — and records
the host process the session runs under, and `SessionEnd`, which records
that the host reported the session ending.) `commonmeasure install claude`
...
of the host's CLAUDE.md instruction files.
