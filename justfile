...
# regenerate a committed run. Neither is in this repository: they need
# COMMONMEASURE_PRIVATE_EVIDENCE set to the directory that holds them
# (CONTRIBUTING.md §Recorded fixtures).

evidence := env_var_or_default("COMMONMEASURE_PRIVATE_EVIDENCE", "")
...
gates: tokens fmt clippy test browser
...
# Refuse a fixtures recipe when the fixture directory is not set
[private]
fixtures:
    @test -n "{{evidence}}" || { echo "this recipe needs COMMONMEASURE_PRIVATE_EVIDENCE, the directory holding recon/ and output/ (CONTRIBUTING.md)" >&2; exit 1; }
    @test -d "{{evidence}}/recon" || { echo "COMMONMEASURE_PRIVATE_EVIDENCE={{evidence}} holds no recon/ directory" >&2; exit 1; }

# Regenerate the committed rubric-ranked replay example (offline: no gateway needed)
rubric-example: fixtures
    cargo run -p commonmeasure-cli -- run demo/jobs/eu-ai-act-replay-rubric.json --replay {{evidence}}/recon --output {{evidence}}/output/latest
    cargo run -p commonmeasure-cli -- inspect {{evidence}}/output/latest

# Replay the recorded providers to a scratch directory (no credentials needed)
replay out="/tmp/commonmeasure-replay": fixtures
    cargo run -p commonmeasure-cli -- run demo/jobs/eu-ai-act-replay.json --replay {{evidence}}/recon --output {{out}}
    cargo run -p commonmeasure-cli -- inspect {{out}}

# Regenerate the committed replay example (gateway: its plans carry answers)
replay-example: fixtures
    cargo run -p commonmeasure-cli -- run demo/jobs/eu-ai-act-replay.json --replay {{evidence}}/recon --output {{evidence}}/output/replay
    cargo run -p commonmeasure-cli -- inspect {{evidence}}/output/replay

# Regenerate the committed cited-run example (gateway: the suite requires citations)
cited-example: fixtures
    #!/usr/bin/env sh
    set -e
    ...
    cargo run -p commonmeasure-cli -- run demo/jobs/eu-ai-act-replay-cited.json --replay {{evidence}}/recon --output {{evidence}}/output/cited
    cargo run -p commonmeasure-cli -- inspect {{evidence}}/output/cited

# Produce a labelled run under the demonstration identity; not committed (gateway; demo/provenance/README.md)
provenance-example: fixtures
    COMMONMEASURE_PROVENANCE_SIGNER=local COMMONMEASURE_PROVENANCE_CERTIFICATE=demo/provenance/signer.pem COMMONMEASURE_PROVENANCE_KEY=demo/provenance/signer-key.pem cargo run -p commonmeasure-cli -- run demo/jobs/eu-ai-act-replay-provenance.json --replay {{evidence}}/recon --output target/demo/provenance
    cargo run -p commonmeasure-cli -- inspect target/demo/provenance
    cargo run -p commonmeasure-cli -- provenance target/demo/provenance/provenance/exa.txt

# Regenerate the committed run over the operator's own corpus (gateway)
internal-corpus-example: fixtures
    COMMONMEASURE_INTERNAL_CORPUS=demo/corpus cargo run -p commonmeasure-cli -- run demo/jobs/energy-price-cap-internal.json --output {{evidence}}/output/tensorzero
    cargo run -p commonmeasure-cli -- inspect {{evidence}}/output/tensorzero

# Regenerate the committed injection-screen run (offline; demo/injection/README.md)
injection-example: fixtures
...
commerce-examples: fixtures
    ...
    for suite in demo/jobs/commerce/*.json; do
        ...
        COMMONMEASURE_INTERNAL_CORPUS="$corpus" cargo run -p commonmeasure-cli -- run "$suite" --output "{{evidence}}/output/commerce/$name"
    done
    cargo run -p commonmeasure-cli -- inspect {{evidence}}/output/commerce/governed-recommendation

# Run the routing experiment's fitting set (offline; demo/jobs/commerce-routing/README.md)
routing-fitting-runs: fixtures
    #!/usr/bin/env sh
    set -e
    ...
    for question in $fitting; do
        for condition in product brand editorial mixed; do
            ...
            COMMONMEASURE_INTERNAL_CORPUS="$corpus" cargo run -p commonmeasure-cli -- run "$suite" --output "{{evidence}}/output/commerce-routing/$condition-$question"
        done
    done
    cargo run -p commonmeasure-cli -- inspect {{evidence}}/output/commerce-routing/mixed-grinder-choice

# Run the routing experiment's holdout set (offline; needs the frozen rule.json)
routing-holdout-runs: fixtures
    #!/usr/bin/env sh
    set -e
    ...
    cargo run -p commonmeasure-cli -- inspect {{evidence}}/output/commerce-routing/product-g4-presto200

# Regenerate the committed licensed-fetch replay (offline: no gateway needed)
redpine-replay-example: fixtures
    cargo run -p commonmeasure-cli -- run demo/jobs/nvidia-redpine-replay.json --replay {{evidence}}/recon --output {{evidence}}/output/redpine-replay
    cargo run -p commonmeasure-cli -- inspect {{evidence}}/output/redpine-replay

# Regenerate the committed skill run (executes local skills; nothing billable; demo/skills/README.md)
skills-example: fixtures
